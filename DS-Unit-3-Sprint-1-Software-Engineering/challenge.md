...
The most important part of a code review to me is answering the question: does it work? This is the most important thing I would pay attention to. If the code does not do what it was intended to do, then there is not point of running tests, othar than to learn how how tests work. In addition to seeing the test cases pass, I would prefer to iteract with the code as a real user would and I would greatly appreciate it if others did the same for me. Regarding containers, things like Docker and virtural environments are great for setting up clean envrionments that are relatively isolated and nicely packaged for deployement. Differing local environments cab be problematic when there is a need for reproduceability and replicability and simply sharing code could result in underministic scenarios. VMs and virtual environments solve this problem by providing a space (a whole operating system or a a few folders) that enables collaboration across slightly different computers with slighlty different things running on them.
